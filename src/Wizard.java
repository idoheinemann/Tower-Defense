...
public class Wizard extends Enemy {
	...
	public boolean move() {
		if(dir[0]==0&&dir[1]==0) {
			...
			for(int i=0;GamePanel.getBlock(getCenterX()+(Block.WIDTH*i*dir[0]), getCenterY()+(Block.WIDTH*i*dir[1])).getType()==Type.ROAD;i++)target = GamePanel.getBlock(getCenterX()+(Block.WIDTH*i*dir[0]), getCenterY()+(Block.WIDTH*i*dir[1]));
		...
		if(GamePanel.getBlock((int)getCenterX(),(int)getCenterY())==target) {
			start = target;
			if(dir[1]!=0) {
				dir[1] = 0;
				if(GamePanel.getBlock(getCenterX()-Block.WIDTH,getCenterY()).getType()==Type.ROAD)dir[0] = -1;
			...
			try {
				for(int i=0;GamePanel.getBlock(getCenterX()+(Block.WIDTH*i*dir[0]), getCenterY()+(Block.WIDTH*i*dir[1])).getType()==Type.ROAD;i++) {
					if(GamePanel.getBlock(getCenterX()+(Block.WIDTH*i*dir[0]), getCenterY()+(Block.WIDTH*i*dir[1]))!=null)break;
